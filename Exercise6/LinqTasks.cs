...
namespace Exercise6
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<Emp> result = Emps.Task12Method();
            return result;
        }
        ...
        public static IEnumerable<Dept> Task14()
        {
            IEnumerable<Dept> result = Depts
                    .GroupJoin(Emps, dept => dept.Deptno, emp => emp.Deptno, (dept, empGroup) => new
                    {
                        Dept = dept,
                        EmpCount = empGroup.Count()
                    })
                    .Where(deptEmp => deptEmp.EmpCount == 0 || deptEmp.EmpCount == 5)
                    .OrderBy(deptEmp => deptEmp.Dept.Dname)
                    .Select(deptEmp => deptEmp.Dept);
            //result =
            return result;
        }
    }
